...
elseif(isset($_POST['Email_inv']) && isset($_POST['Password_inv'])) 
{	
				...
				$Email = validate($_POST['Email_inv']);
				echo $Email;
				$pass = validate($_POST['Password_inv']);
				echo $pass;
				
				$sql = "SELECT * FROM Inventory_Manager WHERE Email_id='$Email' AND Password='$pass'";
         		...
         		if(mysqli_num_rows(mysqli_query($conn,$sql)) === 1) 
				{
					...
					{
						header("Location: Inventory.php?error=Incorect User name or Password");
		        		exit();
					}
					}
			else
			{
				echo $Email;
				header("Location: Inventory.php?error=Incorect User name or Pass");
				exit();
			}
}
...
{
	header("Location:log.php?Invalid User_Name or Password");
	exit();
}
